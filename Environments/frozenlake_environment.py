...
class FrozenLakeEnv(discrete.DiscreteEnv):
    ...
    Transforms:
    is_slippery: Toggle whether there is a chance the agent can "slip" in a direction other than intended
    path_length_penalty: When TRUE, there will be a -1 penalty for each step
    wind: Direction = direction of a wind that pushes agent an extra unit each step. None = no wind.
    ignore_holes: When True, the agent will not be affected by holes in the environment.
    """
    ...
    def __init__(self, desc=None, map_name="4x4", is_slippery=True, path_length_penalty=True, wind=DOWN,
        ...
        def update_probability_matrix(row, col, action):
            ...
            if not path_length_penalty:
                reward = 50 * float(newletter == b'G')
            else:
                reward = 50 * float(newletter == b'G') - 1
