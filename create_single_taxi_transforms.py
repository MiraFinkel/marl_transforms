...
ENV_DIR_PATH = "Transforms/taxi_example_data/taxi_transformed_env/single_transform_envs/"
RESULTS_DIR_PATH = AGENT_DATA_PATH + "results/"
...
def visualize_trained_agent(file_name):
    ...
    file = open(ENV_DIR_PATH + file_name, "rb")
    ...
    new_agent = rl_agent.create_agent(new_env, agent_name)
    ...
    file = open(RESULTS_DIR_PATH + result_dir + result_name + "_explanation.pkl", "rb")
    explanation = pickle.load(file)
    file = open(RESULTS_DIR_PATH + result_dir + result_name + "_result.pkl", "rb")
